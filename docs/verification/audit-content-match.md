...
Real case: while auditing the claim "relative path resolution is at `file.py:291`," the auditor **read only the first two hits** of a repo-wide grep for `file.py` and stopped; line 291 of one of those files **did contain** `def glob(...)`, so the auditor confirmed it as "matches — correct." In fact there were a third and a fourth `file.py` elsewhere, and the claim pointed at one of those. The glob definition at line 291 of the first file was **merely a construct of the same kind that happened to be there**.
