...
さらに条件が3つ重なる: 生成速度が人間のレビュー速度を上回る。実装者が自分のテスト(=自分の採点基準)も書く。複数のエージェントが同一リポジトリ・共有環境で並行作業する。この4条件が、人間チームでは稀にしか起きない失敗形 — **自己過信の工業的な量産** — を日常にする。本ディレクトリの25ドキュメントは、その失敗の実測記録から抽出した対処の体系である。
...
- [完全性掃討の実務](completeness-sweeps.ja.md) — クラスを閉じる列挙の技法(兄弟サイト・単一seam・意味での網羅・不変条件の表・registry軸とseam軸・ツールの過少報告)
