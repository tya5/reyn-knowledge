...
description: 検証ナレッジの体系マップ — 「できました、緑です」という完了報告をマージするまでに、何をどの順で疑うか。13本の配置と、横断する4つの原理
...
flowchart TD
    ...
    S5["Stage 5: 申告の出自<br/>成果物は実体から来たか"] --> M["マージ"]
```
...
### Stage 5 — 申告の出自: 成果物(artifact)は実体から来たか
...
- **レビュー・co-vet(独立第二検証)する側**: [一括修正 PR](sweep-reviews.ja.md) → [fix-class レビュー](fix-class-review.ja.md) → [共有ヘルパー](shared-helper-widening.ja.md) → [strip 計測器](strip-instrument-integrity.ja.md)。diff と報告の外側を見る技術。
