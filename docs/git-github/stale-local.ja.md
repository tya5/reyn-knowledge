...
`gh pr merge`(GitHub CLI でのサーバ側マージ)は **origin/main(リモートの正)を進めるが、手元の checkout には何もしない**。人間の開発では「マージしたら pull する」が自然に挟まるが、エージェントが1セッションで何本も PR をサーバ側マージしていくと、**ローカルの main は気づかないうちに5〜15コミット遅れる**。その古い木に対する grep・診断・行番号・分岐(ブランチ作成)が、以下の事故を量産する。

これは[測定対象のずれ](../verification/measurement-target.ja.md)・[strip(検証のため一時的にコードを無効化する操作)計測器の健全性](../verification/strip-instrument-integrity.ja.md)の git 版である: **「自分が見た木」と「効く木」(実際に効力を持つ側 = origin/main)は別物**で、しかもズレは**時間とともに黙って開く**。

## 事故1 — stale な grep が、偽の指摘と偽の安心を生む

実例(偽の指摘): レビュアーが完全性の掃討(「同じクラスの未修正兄弟は他に無いか」)をローカルの木に grep を実行し、ヒットした箇所を「未修正の兄弟」として修正要求に載せた。**そのシンボルは5コミット前に削除済み**だった。実装者が `git show origin/main:<path> | grep -c ...` → 0 で反証。健全な PR に偽の指摘を投げる形で、[strip 計測器](../verification/strip-instrument-integrity.ja.md)の「健全なゲートへの誤告発」と同じ損害である。
...
> **完全性・「ここに在る/無い」系の主張の grep は、ローカルではなく、`git fetch origin main` を先に実行してから、`git show origin/main:<path>` か `git grep <パターン> origin/main` で実行する。** 通常の探索なら stale でも実害は小さいが、「これで全部」「ここにも在る」という主張は、**木が古いだけで偽になる**。

付随: 挙動確認の `python -c "import mypkg; ..."` も**別の clone を解決していることがある**(`mypkg.__file__` で解決先を確認するまで信用しない — [strip 計測器](../verification/strip-instrument-integrity.ja.md)壊れ方3(実行環境が別のツリーを解決している形))。
...
さらに一般化される(worktree(並行チェックアウト)へのディスパッチ(作業割り当て)に限らない): **docs だけの PR でも起きる**。実例では、9コミット遅れの detached HEAD(どのブランチにも乗っていない checkout 状態)から切った「+111行のドキュメント PR」の diff が、実は **−789行 = 9コミット分の巻き戻し**を含んでいた。「+N docs」という見出しは additive(追加のみ)の証明にならない。

- 分岐前に同期: `git fetch origin main && git merge --ff-only origin/main`。実装側にも開始時の `git fetch origin main && git rebase origin/main` を依頼に含める。
- **マージ前の警戒条件(tripwire)**: 小さいはずの PR に予期しない削除・大きな負の行数がある、または `git merge-base <branch> origin/main` が現在の origin/main と一致しない → stale base。マージせず rebase して diff を取り直す。
...
- 根本対処は**置き場所**: エージェントは共有ツリーで作業せず、自分専用の clone / worktree を持つ。
...
長時間の分析(数百件の行番号つき分類)を続けている間に、共有ツリーが別の作業で pull され、対象ファイルが**+107行**動いた実例がある。分析データは全て古いスナップショット基準になっていた。救ったのは**適用直前の内容ベース再アンカー**(行番号の指す内容が期待と一致するかの照合)で、照合に使った100件のうち87件の不一致が出て発覚 — 照合がなければそのまま適用してファイルを破壊していた。
...
- [ ] 完全性・存在の主張の grep を `origin/main` 参照で実行したか(ローカル木ではなく)
