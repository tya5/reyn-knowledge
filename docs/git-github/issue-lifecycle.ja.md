...
description: issue は劣化する — 本文は起票時のスナップショットで、解決はスレッドと merged PR 側に溜まる。dispatch 前の照合、クローズの検証記録、残件の決着(「次アークで」は第三の状態)、起票の較正(両方向、題が機構を断定する罠を含む)まで
...
**題(タイトル)は前提として読まれる — 機構が未確定なら書かない。** 起票者が題に機構の候補まで書くと(「read-before-write レース」のように)、担当者はその**枠の中で**答えを探しがちになり、枠自体が誤っていれば「枠内で最良の答え」が誤りごと採用される。実測: ある issue を「read-before-write のレース」と題して起票し、本文には推論であるとラベルを付けていたが、**題は断定形のままだった**。担当者は題を前提とせず、まず枠そのものを反証した — 読み手側のコードは全て存在ガード付きで、引用したスタック行も実は読み経路ではなく別処理の再送出だったと判明し、真因は**書き手側の削除後の書き込み**(2つ目のテストインスタンスが1つ目の生きたツリーを消していた)だった。

- 起票の題は、**機構が確定していない限り症状までに留める**。「〜というレース」でなく「〜という症状」と書く。
- スタックの行番号を機構の証拠に使うなら、**その行を実際に開く**。件数や行番号を証拠として引用し、開かずに済ませるのは[列挙の規律](../verification/enumeration-discipline.ja.md)と同じ怠りである。
- 本文に INFERRED(推論)のラベルを付けても、**題には効かない** — 題は誰もが最初に読み、前提として扱う面である。ラベルは本文の中でしか主張の身分を守らない。
