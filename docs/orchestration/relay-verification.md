...
In fleet operation (running multiple agents concurrently), claims flow in through a message hub — a relay that carries messages between sessions. A claim that arrives through the hub **looks like a fact but is, in substance, hearsay**. And relaying amplifies: the moment the lead carries it to the owner, the hearsay **wears the face of a first-hand report** (the receiver trusts the lead's report as already verified). This document takes the labeling discipline of [Claims Without Context](../verification/cross-context-claims.md) and specializes it to the act of relaying.
...
- Whether a service is alive, its port, its health, its process: **one shot of curl / lsof / ps** settles it (e.g. `curl -s -o /dev/null -w '%{http_code}' http://localhost:8000/health` — this one line settles the "down" hypothesis). At that price, there is no excuse for skipping verification.
...
A claim is carried together with the literal command (the exact command that was run) and its scope (what was checked). Shorthand claims like "audit clean," "CI green," or "trigger condition met" have caused a measured propagation accident across a three-session chain: the originator had only run a narrow check (`--check private-state` — a single-rule check), but the report to the hub was written as the broad check (`--strict tests/` — all rules — at exit 0), the relayer forwarded it unverified, and **only when the receiver ran the literal command directly did 18 errors surface** — the narrow/broad gap was in the kind of check itself, not the target directory.
...
- **As receiver**: if the claim **gates your next action** (serves as the checkpoint that decides go/stop), run the quoted literal command yourself before moving.
...
Measured case: a report that "12/12 empty stalls on unknown actions = structural blocker" was carried to the owner as-is, as a "deep structural defect." In reality, the test had been built from **fake components plus tasks that made no sense**, which had **amplified a real but partial defect (an empty-stall rate of about 67% on real tasks) into total failure (100%)**. There is a symmetric trap on the other side, once the confound comes to light: overcorrecting to "pure artifact (no defect at all)" — the true value was in between.
