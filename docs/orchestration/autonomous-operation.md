...
2. **As judgment**: "fatigue," "quality," "the session is long" are not numbers. The only admissible input is the **actual context usage rate**. Measured: when "it's major surgery, so a fresh context" was proposed, actual usage was **about 30% of the 1M-token limit**. The owner pushed back twice ("you're giving up too early"; "show evidence, or prevent the recurrence").

The rule cuts **both ways**:

- **Deferring below roughly 60–70% is overcaution** — keep going. If the other party's "checkpoint (a pause at a clean stopping point) / fresh context / hard part with a fresh head" carries **no number, reject it on the spot and instruct them to continue**. Measured: the lead who had recorded this very lesson endorsed a numberless deferral themselves and was called out by the owner (recording a lesson and applying it are different operations).
